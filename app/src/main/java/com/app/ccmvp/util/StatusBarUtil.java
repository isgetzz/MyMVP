...
public class StatusBarUtil {
    ...
    public static final void setLucency(Window window) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            window.setStatusBarColor(Color.TRANSPARENT);//状态栏
            //window.setNavigationBarColor(Color.TRANSPARENT);//虚拟键盘、home键、返回键
    ...
    //信息栏跟图片适配
    public static final void setAll(Window window) {
        // 状态栏字体黑色 SYSTEM_UI_FLAG_LIGHT_STATUS_BAR、白色 SYSTEM_UI_FLAG_LAYOUT_STABLE
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
...
*/
    ...
    //半屏透明适配详情页图片
    public static void immersiveScreen(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
    ...
    public static void immersive(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            if (color == 0xFFFFFFFF) {
                ...
                //设置白色 View.SYSTEM_UI_FLAG_VISIBLE  黑色 SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            darkModeForFlyme4(window, color == 0xFFFFFFFF ? true : false);
            immersive(window, color, alpha);
        } else if (isMIUI6Later()) {
            darkModeForMIUI6(window, color == 0xFFFFFFFF ? true : false);
            immersive(window, color, alpha);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            darkModeForM(window, color == 0xFFFFFFFF ? true : false);
            immersive(window, color, alpha);
        } else if (Build.VERSION.SDK_INT >= 19) {
            //版本6.0以下
            ...
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                window.setStatusBarColor(0xFF262521);
            }
    ...
    public static int getStatusBarHeight(Context context) {
        int result = context.getResources().getDimensionPixelOffset(R.dimen.x13);
        ...
        //根据目前需求 如果小于5.0不设置状态栏
        return Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP ? 0 : result;
    }
}
